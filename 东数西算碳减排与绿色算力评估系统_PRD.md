...
|:---|:---|:---|:---|
| 2026-05-13 | 修复桌面软著源程序鉴别材料中的乱码：使用 UTF-8 安全方式重新生成源程序代码章节，恢复“源程序代码”“前30页源代码”“文件：”等中文标题与代码注释正常显示，并保留乱码修复前备份；已校验 `???`、替换字符、常见错误编码片段和外部 URL 泄露均为 0 | 桌面 `东数西算软著材料`; 源程序鉴别材料 docx; 本 PRD | 后续用脚本重生成 docx 时必须避免 PowerShell 管道编码破坏中文，优先使用 ASCII 脚本常量或 UTF-8 文件方式 |
| 2026-05-13 | 已同步更新桌面软著材料《东数西算碳减排与绿色算力评估系统-源程序鉴别材料.docx》：重新生成源程序代码章节的前30页与后30页，代码片段同步到当前内部重构后的 `src/lib/auth.ts`、`backend/auth_store.py`、`backend/live_scraper.py` 写法，并更新源码清单行数；已保留修改前备份 | 桌面 `东数西算软著材料`; 源程序鉴别材料 docx; 本 PRD | 后续每次代码内部重构后，如软著材料仍需保持一致，应同步重生成源程序鉴别材料并保留备份 |
| 2026-05-13 | 认证与实时数据模块做语义等价内部重构：后端爬虫与认证存储改用普通 `for` 循环和局部临时变量，前端认证兜底逻辑改为显式循环与保守短变量名；不改变接口字段、返回结构、页面布局、样式和业务结果；对应 Git 检查点为本次提交 | backend/live_scraper.py; backend/auth_store.py; src/lib/auth.ts | 后续调整内部重构、认证兜底、实时数据派生结构或接口字段时，需确认页面与 API 行为保持兼容，并同步本记录 |
